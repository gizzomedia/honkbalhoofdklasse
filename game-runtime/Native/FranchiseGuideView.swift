...
extension FranchiseView {
    var tourSteps:[FranchiseTourStep] {[
        .init(tab:0,title:"WELCOME TO YOUR FRANCHISE",body:"This is your matchday overview: the next opponent, projected starters, recent results and club objectives. Board expectations differ by club and adapt after each season; meeting three goals earns an extra development grant at rollover. Your career is paused during this tour. Nothing in your squad or finances is changed."),
    ...
    func managerLeagueRosters(){
        ...
        for (i,p) in players.dropFirst(leagueRosterPage*10).prefix(10).enumerated(){let y:CGFloat=450+CGFloat(i)*32,r=rect(51,y,1493,31)
            ...
            text(String(p.rating),775,y+1,20,selected.highlight,"AvenirNextCondensed-Heavy",88);if let first=p.developmentHistory?.first(where:{$0.year==f.year}){let delta=p.overallValue-first.overall;text(String(format:"%+.1f",delta),820,y+7,13,delta>0.05 ? green:muted,"Menlo",65)};text(p.profile.birthYear.map{String(f.year-$0)} ?? "—",892,y+4,20,white,"Menlo",84);text("\(p.profile.bats ?? "—") / \(p.profile.throws ?? "—")",992,y+5,18,muted,"Menlo",112)
