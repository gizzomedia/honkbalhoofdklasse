...
export class Renderer {
  ...
  paint(frame) {
    this.frame=frame;this.cache.begin(frame);const c=this.ctx;c.setTransform(this.canvas.width/1600,0,0,this.canvas.height/900,0,0);c.clearRect(0,0,1600,900);
    for(const cmd of frame.commands||[]) {
      const [op,...a]=cmd;
      if(op==='fill'){c.fillStyle=a[4];c.fillRect(...a.slice(0,4));}
      else if(op==='text')this.text(cmd);
      else if(op==='image'){const [path,x,y,w,h,alpha,cover,cy]=a,loaded=this.cache.get(path,[x,y,w,h],cy,frame.page);if(!loaded)continue;const im=loaded.image;
        ...
        c.save();c.beginPath();c.rect(x,y,w,h);c.clip();c.globalAlpha=alpha;c.drawImage(im,x+w/2-iw/2,y-(ih-h)*loaded.cy,iw,ih);c.restore();
