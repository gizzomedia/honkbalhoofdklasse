...
await click('group:3');await click('tab:6');await expect(page.locator('[data-action="tradestrict"]')).toBeVisible();await click('tradestrict');
...
await click('tab:16');await page.screenshot({path:`../work/web-parity/${engine}-sponsors.png`});
...
const inspected=JSON.parse(saved);if(inspected.tradeHistory?.length!==1 || inspected.settings?.tradeStrictness!==2)throw Error('Trade decision or GM strictness was not saved');
