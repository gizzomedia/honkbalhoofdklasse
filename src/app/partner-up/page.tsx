...
const IG_POSTS = [
  {
    image: 'https://res.cloudinary.com/dqld625sq/image/upload/v1778542430/logo_hk_abi5hm.png',
    caption: 'Wat een week in de Hoofdklasse! Nando Mostaert pakt de Player of the Week award met een fenomenale prestatie. 5 hits, een homerun en 4 RBI in één serie. 🏆⚾',
    reach: 8420,
    likes: 724,
    comments: 38,
    saves: 91,
  ...
  {
    image: 'https://res.cloudinary.com/dqld625sq/image/upload/v1778542430/logo_hk_abi5hm.png',
    caption: 'Neptunus vs Pirates — de klassieker van de Hoofdklasse. Bekijk de samenvatting en stats op honkbalhoofdklasse.com. Link in bio. 🔗',
    reach: 11240,
    likes: 1103,
    comments: 67,
    saves: 148,
    ...
    <div className="max-w-sm mx-auto">
      ...
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-[var(--border)]">
          <div className="w-9 h-9 rounded-full bg-[var(--accent)] flex items-center justify-center shrink-0 p-1">
        ...
        {/* Bereik highlight */}
        <div className="bg-gradient-to-r from-[var(--accent)]/20 to-transparent px-4 py-3 flex items-center gap-3 border-b border-[var(--border)]">
          <div>
            <p className="font-display font-700 text-[10px] uppercase tracking-widest text-[var(--accent)]">Bereik</p>
            <p className="font-display font-800 italic text-2xl text-white leading-none"><strong>{post.reach.toLocaleString('nl-NL')}</strong></p>
          </div>
          <div className="h-8 w-px bg-white/10" />
          <p className="font-display font-700 text-[10px] text-[var(--muted)] uppercase tracking-wider leading-tight">accounts<br/>bereikt</p>
        </div>
        ...
        <div className="grid grid-cols-3 border-t border-[var(--border)]">
          {[
            { icon: <svg viewBox="0 0 24 24" className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" /></svg>, val: post.likes },
            { icon: <svg viewBox="0 0 24 24" className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z" /></svg>, val: post.comments },
            { icon: <svg viewBox="0 0 24 24" className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0z" /></svg>, val: post.saves },
          ].map(({ icon, val }, si) => (
            <div key={si} className="flex items-center justify-center gap-1.5 py-3 text-white/50">
              {icon}
              <span className="font-display font-700 text-xs">{val.toLocaleString('nl-NL')}</span>
            </div>
          ))}
        </div>
      </div>
